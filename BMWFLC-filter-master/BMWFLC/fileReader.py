...
def readFromFileLog3():
    # Action tremor LH  Accompanies voluntary LH P/S
    # x: RH measuring P+/S  deg/sec
    # y: LH measuring P+/S  deg/sec

    data = []
    with open("TremorData/log3.txt", "r") as csvfile:
        readCSV = csv.reader(csvfile, delimiter=',')
        for row in readCSV:
            data.append([float(i) for i in row])
    data = list(zip(*data))
...
def readFromTestUtenNr(nr):
    data = []
    with open(f"DVAtest/Test{nr}/Uten masse.txt", "r") as csvfile:
        readCSV = csv.reader(csvfile, delimiter=',')
        for row in readCSV:
            data.append([float(i) for i in row])
    data = list(zip(*data))
...
def readFromTestMasseNr(nr):
    data = []
    with open(f"DVAtest/Test{nr}/Med masse.txt", "r") as csvfile:
        readCSV = csv.reader(csvfile, delimiter=',')
        for row in readCSV:
            data.append([float(i) for i in row])
    data = list(zip(*data))
...
def readFromTestDempingNr(nr):
    data = []
    with open(f"DVAtest/Test{nr}/Med demping.txt", "r") as csvfile:
        readCSV = csv.reader(csvfile, delimiter=',')
        for row in readCSV:
            data.append([float(i) for i in row])
    data = list(zip(*data))
